...
function Connecter() {
    ...
    const fetchElement = async ()=>{
        const response = await fetch("localhost:4400");
  ...
  return (
    <div class="overflow-hidden p-10 w-full h-screen md:items-center flex flex-row text-gray-600 body-font">
            ...
            <div class=" md:w-1/2 h-full justify-center bg-gray-100 rounded-lg p-8 flex flex-col w-full mt-10 md:mt-0">
                ...
                <button class="text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg" onClick={ajout()}>Se connecter</button>
            </div>
        </div>
  );
}
